...
def plot_box_plots(
    ...
    plot = (
        ...
        + p9.geom_boxplot(p9.aes(x=variable_id_map[var], y=var, fill=variable_id_map[var]), outlier_shape='')

        )
    if measurements[var].empty is False:
        plot += p9.geoms.geom_point(
            p9.aes(y="measurement", x=variable_id_map[var]),
...
def plot_oos(maud_oos_dir, output_dir):
    ...
    flux = flux_df.groupby(["experiments", "reactions"]).mean().reset_index()[["experiments", "reactions", "flux"]].rename({"flux": "mean"}, axis=1)
    flux["var"] = flux_df.groupby(["experiments", "reactions"]).var().reset_index()["flux"]
    flux["explore"] = np.abs(flux["var"] / flux["mean"])
    flux_exploration = flux.groupby(["experiments"]).mean().reset_index()[["experiments", "explore"]]
    exploration_plot = (ggplot(flux_exploration)
     + aes(x='experiments', y="explore")
     + geom_point(size=2)
     + labs(y='var / mean', x='experiment', title='Average exploration for each experiment condition')
    )
