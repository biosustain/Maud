...
for rxn in mi.kinetic_model.reactions.values():
    ...
    tmp_formation_energies = {
        mic: formation_energies[formation_energies["metabolite_id"] == mic[:-2]]["location"].values[0]
    ...
    for _, row in tmp_stoic.iterrows():
        tmp_dG += 0.008314*298.15*row["stoichiometry"]*tmp_formation_energies[row["mic_id"]]
    ...
    for enz_id, enz in rxn.enzymes.items():
        ...
        tmp_enz_entry = tmp_kms.merge(tmp_stoic, left_on=['mic_id'], right_on=['mic_id'])
        ...
        substrate_entry = zip(substrate_information['mic_id'], substrate_information['id'], substrate_information['stoichiometry'].abs())
        metabolite_denomintor = zip(tmp_enz_entry['mic_id'], tmp_enz_entry['id'], tmp_enz_entry['stoichiometry'].abs())
        product_entry = zip(product_information['mic_id'], product_information['id'], product_information['stoichiometry'].abs())
        ...
        if allosteric_inhibitors == []:
            Allo_Inh = "1"
        if len(tmp_L) > 0:
            Allo = Template_Allo.render(L0=tmp_L["id"].values[0],
                                        Dr=Dr,
                                        Drreg=Drreg,
                                        Allo_Inh=Allo_Inh,
                                        Allo_Act=Allo_Act,
...
par_input = par_input + [[par.id, par.location] for par in mi.priors.inhibition_constant_priors]
