...
transformed parameters {
  ...
  real scaled_concentration_training[N_balanced+N_unbalanced, N_training_experiments];
  ...
  for (e in 1:N_training_experiments){
    for (m in 1:(N_unbalanced+N_balanced)){
      scaled_concentration_training[m, e] = concentration_training[m, e]/scaling_factor[m];
    }
  }
}
model {
  ...
  for (e in 1:N_experiment){
    prior_location_unbalanced[,e] ~ lognormal(log(scaled_unbalanced[,e]),
  ...
  if (LIKELIHOOD == 1){
    ...
    for (mc in 1:N_concentration_measurement_training){
      concentration_hat[mc] = scaled_concentration_training[ix_training_concentration_measurement[mc],
...
generated quantities {
  ...
  real scaled_concentration_holdout[N_balanced+N_unbalanced, N_holdout_experiments];
  ...
  for (e in 1:N_holdout_experiments){
    for (m in 1:N_unbalanced+N_balanced){
      scaled_concentration_holdout[m, e] = concentration_holdout[m, e]/scaling_factor[m];
    }
  }
