...
import click
import cmdstanpy
from enzymekat import sampling, simulation, sampling_CV, sampling_relative_CV, sampling_relative
import os
...
def sample_relative(data_path, **kwargs):
    """Sample from the model defined by the data at data_path."""
    stanfit = sampling_relative.sample(data_path, **kwargs)
    print(stanfit.summary())
...
@click.argument('data_path', type=click.Path(exists=True, dir_okay=False),
                default=get_example_path(RELATIVE_PATH_EXAMPLE))
def sample_relative_CV(data_path, **kwargs):
    """Provides cross validation for relative model inputs"""
    stanfit = sampling_relative_CV.sample(data_path, **kwargs)
    print(stanfit.summary())
