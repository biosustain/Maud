...
from pydantic import root_validator, StrictInt, StrictFloat

from maud.utils import recursively_flatten_sequence
...
class StanData:
    ...
    def no_nans_allowed(cls, values):
        ...
        if isinstance(values["value"], Sequence):
            flat = recursively_flatten_sequence(values["value"])
...
class StanInput:
    ...
    def __post_init__(self):
        ...
        self.N_unbalanced = StanData(
            name="N_unbalanced", value=len(self.priors_conc_unbalanced.value[0][0])
        )
        ...
        self.N_reaction = StanData(
            name="N_reaction",
            value=(
                len(self.priors_kcat.value[0])
                + len(self.priors_drain.value[0][0])
            )
        )
        ...
        self.N_experiment = StanData(
            name="N_experiment", value=len(self.priors_conc_unbalanced.value[0])
        )
