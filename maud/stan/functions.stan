functions {
  ...
  matrix get_flux_control_matrix(matrix S,
    ...
    return I - elasticity*generalized_inverse(balanced_S*elasticity)*balanced_S;
  }
  complex_vector get_complex_edge_flux_metabolite(complex_vector conc,
    ...
    int N_edge = cols(S);
    vector[N_edge] vmax = get_vmax_by_edge(enzyme, kcat, edge_to_enzyme, edge_type);
    complex_vector[N_edge] reversibility = get_reversibility_complex(dgr, temperature, S, conc, edge_type);
    complex_vector[N_edge] free_enzyme_ratio = get_free_enzyme_ratio_complex(conc,
                                                             ...
                                                             ci_ix_long,
                                                             ci_ix_bounds);
    complex_vector[N_edge] saturation = get_saturation_complex(conc,
                                               km,
                                               free_enzyme_ratio,
                                               sub_km_ix_by_edge_long,
                                               sub_km_ix_by_edge_bounds,
  ...
  complex_vector get_allostery_complex(complex_vector conc,                               // one per mic
                       complex_vector free_enzyme_ratio,                  // one per edge
                       vector tc,                                 // one per allosteric enzyme
                       vector dc,                                 // one per allostery
                       vector subunits,                           // one per edge
                       array[] int allostery_ix_long,             // - long and bounds encode a ragged
    ...
    for (f in 1:N_edge){
      ...
      out[f] = 1/(1 + tc_edge * (free_enzyme_ratio[f] * Q_num / Q_denom) ^ subunits[f]);
    }
    return out;
  }


    complex_vector get_reversibility_complex(vector dgr,
                                             real temperature,
                                             matrix S,
                                             complex_vector conc,
                                             int[] edge_type){
  ...
  complex_vector get_free_enzyme_ratio_complex(complex_vector conc,
                               matrix S,
                               vector km,
                               vector ki,
                               int[] edge_type,
  ...
  complex_vector get_saturation_complex(complex_vector conc,
                        vector km,
  ...
  complex_vector get_drain_by_edge_complex(vector drain,
                           complex_vector conc,
                           int[] edge_to_drain,
                           int[] sub_by_edge_long,
                           int[,] sub_by_edge_bounds,
